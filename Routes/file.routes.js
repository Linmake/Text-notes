...
FileRouter.post('/create/:idFolder/', validateFile, async (req, res) => {
  try {
    ...
    if (!project) return res.status(400).send(`Project con Id: "${folder.IdProyect}" no existe`);

    const folderRef = project.Folders.find(f => f.Id == idFolder);
    if (!folderRef) {
      return res.status(400).send(`Folder dentro del project con Id: "${idFolder}" no existe`);
    }
...
FileRouter.delete('/delete/:idFolder/:idFile/', async (req, res) => {
  try {
    ...
    if (!folderInProyect) return res.status(400).send(`Folder con Id: "${idFolder}" no existe dentro del project`);
